...
import { useChainSentinelReward } from '../useSmetReward';

describe('useSmetReward', () => {
  it('throws when write is not ready', async () => {
    const { result } = renderHook(() => useChainSentinelReward());
  ...
  it('prepares args with paymentInNative and poolId', () => {
    ...
    renderHook(() => useChainSentinelReward({ paymentInNative: false, poolId: 1 }));
  ...
  it('defaults poolId to 0 when not provided', () => {
    ...
    renderHook(() => useChainSentinelReward({ paymentInNative: true }));
    const lastCall = wagmi.usePrepareContractWrite.mock.calls[0][0];
